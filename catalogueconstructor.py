...
class CatalogueConstructor:
    ...
    **Persitent atributes**:
    ...
    def delete_waveforms_cache(self):
        # Eliminar referencias a memmap
        if hasattr(self, 'some_waveforms'):
            del self.some_waveforms
        if hasattr(self, 'some_waveform_index'):
            del self.some_waveform_index
        if hasattr(self.arrays, 'some_waveforms'):
            del self.arrays.some_waveforms
        if hasattr(self.arrays, 'some_waveform_index'):
            del self.arrays.some_waveform_index
        # Eliminar de la colección interna de arrays (ArrayCollection)
        if hasattr(self.arrays, '_arrays'):
            self.arrays._arrays.pop('some_waveforms', None)
            self.arrays._arrays.pop('some_waveform_index', None)
        import gc
        gc.collect()
    ...
    def make_catalogue(self,
        ...
        # Asegurar que some_waveforms está disponible
        if not hasattr(self, 'some_waveforms') or self.some_waveforms is None:
            self.cache_some_waveforms()
        ...
        self.catalogue['clean_peaks_params'] = dict(self.info['clean_peaks']) if 'clean_peaks' in self.info else {}
